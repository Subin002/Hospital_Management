...
function EmpUpdate() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:1300/empbyid/${id}`);
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:1300/updateemp/${id}`, formData, config);
            navigate('/empdata');
        } catch (error) {
    ...
    return (
        <div className='container'> 
            <div className='home-container'>
                <h2>EMPLOYEE FORM</h2>
                <form onSubmit={handleUpdate} className='form02'>
